...
from webcam_processing import process_webcam_frame
import cv2
import threading
import time
...
def diagnose_webcam():
    ...
    while True:
        ...
        if success:
            try:
                diagnosis = process_webcam_frame(frame)  # 프레임 진단
                with lock:
...
def get_diagnosis():
    ...
    with lock:
        if not latest_diagnosis:
            return jsonify({"status": "no_result", "message": "No diagnosis available"}), 204
        return jsonify({"status": "success", "results": latest_diagnosis})
